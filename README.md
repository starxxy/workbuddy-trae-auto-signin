...
**WorkBuddy / CodeBuddy / Trae CN / Trae Code 多平台自动签到脚本（单文件 · 零第三方依赖）**

一个自用的多平台签到小工具：一份 Python 标准库代码，同时照顾 WorkBuddy / CodeBuddy 与 Trae CN / Trae Code 四个客户端的每日签到与积分领取，并附带一个本地网页控制台。
...
- **多平台，一份代码**：WorkBuddy + CodeBuddy（共用腾讯 copilot 引擎）+ Trae CN + Trae Code（共用 Trae 引擎）。
...
powershell -ExecutionPolicy Bypass -File .\install-windows.ps1 -Platform both # 两个都装（推荐）
...
# 双平台
python signin.py both           # 两个平台都签
python signin.py both silent    # 静默版本
```
...
| 模块 | 能干什么 |
|---|---|
| **状态与积分总览** | 四个平台并列显示（2×2 等高卡片）：今日是否已签、客户端是否检测到、累计积分、连续天数、活动是否启用、凭据是否有效 |
| **一键签到 / 补签** | 「签 WorkBuddy」「签 CodeBuddy」「签 Trae CN」「签 Trae Code」「全部一起签」「只跑成长中心」按钮，执行中显示加载态，结果内联展示（区分 `ALREADY` / `CLAIMED` / `NO_AUTH` / `ERROR`） |
| **日志查看与筛选** | 读真实 `signin.log`，最新在前，可按平台和结果类型筛选，补签轮询行（`trigger: poll`）单独标记；支持手动刷新与自动轮询开关 |
| **定时任务管理** | 列出本机三个计划任务的真实状态与下次运行时间，可启停，可改签到时间；尚未安装时直接给出安装命令 |
...
| 方法 | 路径 | 作用 |
|---|---|---|
| `GET` | `/api/status` | 四平台状态、积分汇总与客户端检测结果 |
| `GET` | `/api/health` | 健康检查与运行环境信息 |
| `POST` | `/api/signin/<platform>` | 签到，`platform` 取 `workbuddy` / `codebuddy` / `trae` / `traecode` / `both`（`both` 按引擎各签一次，避免同体系重复领取） |
| `POST` | `/api/growth` | 单独跑一次 WorkBuddy 成长中心全套 |
| `GET` | `/api/logs` | 读日志，支持 `limit` / 平台 / 结果类型筛选 |
| `GET` | `/api/tasks` | 三个计划任务的安装状态、启停状态、下次运行时间 |
...
- **后端不复制签到逻辑**：网页的所有查询与领取都直接 `import signin` 调用其中的函数（`run_daily` / `run_growth` / `trae_status` / `trae_claim` 等）。接口地址、重试、幂等判定、错误归类只有一份实现，所以**命令行和网页的行为永远一致**，不会出现「CLI 说已签、网页说还能领」的分叉。
...
| 想看两个平台的原始返回 | `python signin.py status` 和 `python signin.py trae status` |
...
- 永远不会打印 `accessToken` 或 Trae JWT，`Authorization` 头不会出现在日志里。
...
```
workbuddy-trae-auto-signin/
├── signin.py                          # 主脚本（WorkBuddy + Trae CN，纯标准库；openssl 可选，缺失时用内置纯 Python AES）
├── webapp.py                          # 本地 Web 控制台后端（Flask，只做编排、复用 signin.py）
├── requirements.txt                   # Web 控制台的依赖（只有 Flask）
├── web/
│   └── index.html                     # 控制台前端（单文件，内联 CSS/JS，无外部资源）
├── install-windows.ps1                # Windows 一键安装（-Platform workbuddy|trae|both）
└── README.md                          # 本文档
```
